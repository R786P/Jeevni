...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadFilteredStories() {
        let query = supabase.from('Jeevni').select('*').order('created_at', { ascending: false });
    ...
    async function loadAllStories() {
        const { data, error } = await supabase.from('Jeevni').select('*').order('created_at', { ascending: false });
        if (error) {
    ...
    async function loadStories() {
        const { data, error } = await supabase.from('Jeevni').select('*').order('created_at', { ascending: false }).limit(3);
        if (error) {
